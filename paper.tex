...
$N = 64$ ($m = 6$ qubits). For each pattern, we report
the raw gate count from PyEncode's synthesiser, the
count after Qiskit transpilation to $\{$CX, U, X, H$\}$
at optimisation level~2, and the transpiled Shende
\texttt{StatePreparation}~\cite{shende2006} count on the
same vector.

\begin{table*}[ht]
\centering
\caption{Gate counts at $N=64$ ($m=6$). PyEncode columns show raw synthesiser output and
after Qiskit transpilation to $\{CX,U,X,H\}$ (opt.\ level~2).
Shende applies \texttt{StatePreparation}~\cite{shende2006} to the same vector, transpiled identically.}
...
\textbf{Pattern} & Raw & Transpiled & $\mathcal{O}(\cdot)$
                 & Transpiled & $\mathcal{O}(\cdot)$ \\
\midrule
Discrete ($k\!=\!20$)
