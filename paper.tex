...
written as \texttt{f = np.sin(np.pi * x / L)}, produces
...
f = np.sin(np.pi * x)
for i in range(N):
    f[i] = np.sin(np.pi*i/N)
f = np.array([np.sin(np.pi*i/N) for i in range(N)])
...
\begin{lstlisting}[style=python]
f = np.sin(3*np.pi*x + np.pi/4)        # pure sinusoid
f = np.sin(3*np.pi*x) + 0.1            # DC offset: different circuit
f = np.sin(3*np.pi*x) * np.exp(-x)     # damped: DFT no longer sparse
f = np.sin(3*np.pi*x) + np.sin(5*np.pi*x)  # two modes: different class
\end{lstlisting}
...
and phase $\varphi$:
\[
  f_i = A\sin\!\left(\frac{n\pi i}{N} + \varphi\right)
\]
...
\[
  f_i = A\cos\!\left(\frac{n\pi i}{N} + \varphi\right)
\]
...
\[
  f_i = \sum_{t=1}^{T} A_t \sin\!\left(
        \frac{n_t \pi i}{N} + \varphi_t\right)
\]
Constructor:
...
Sine             & \texttt{SINE(n,A,phi)}           & $A\sin(n\pi i/N+\varphi)$             & $\mathcal{O}(m^2)$ \\
Cosine           & \texttt{COSINE(n,A,phi)}         & $A\cos(n\pi i/N+\varphi)$             & $\mathcal{O}(m^2)$ \\
Multi-discrete   & \texttt{MULTI\_DISCRETE(...)}    & $\sum_j P_j\delta_{ik_j}$             & $\mathcal{O}(|S|m)$\\
Multi-sine       & \texttt{MULTI\_SINE(...)}        & $\sum_t A_t\sin(n_t\pi i/N+\varphi_t)$ & $\mathcal{O}(m^2)$ \\
\bottomrule
\end{tabular}
\end{table*}
...
\loadfigs{sine_loop_vector.png}{sine_loop_circuit.png}
  {Sine vector: $\sin(3\pi i/N)$, $N=64$}
  {unused}
  {0.5}{sec5_sine_loop}

\textbf{Example 3: Checking for Shende fallback.}
When the pattern is not recognised and no hint is
provided, PyEncode falls back to Shende's general
preparation and issues a warning. The \texttt{info}
object allows the caller to detect this
programmatically:
\begin{lstlisting}[style=python]
import warnings
...
if info.complexity == "O(2^m)":
    warnings.warn(
        f"Shende fallback: gate count "
        f"{info.gate_count} for N=64")
\end{lstlisting}

\loadfigs{damped_sine_vector.png}{damped_sine_circuit.png}
  {Damped sine: $\sin(3\pi x)e^{-x}$ (Shende fallback)}
  {unused}
  {0.35}{sec5_damped}

The damped sinusoid is not in PyEncode's pattern
library; the warning alerts the user that the
exponential-cost circuit was used and invites them
to either supply a \texttt{vector\_type} hint or
...
\textbf{Example 1: Auto-detection, unknown source.}
A FEM-assembled vector passed directly. PyEncode tries
each extractor in preference order and returns the
best-matching pattern without the user needing to know
PyEncode's vocabulary:
\begin{lstlisting}[style=python]
from pyencode import encode_vector

f = assemble_vector(mesh)   # any FEM assembler
circuit, info = encode_vector(f)
print(info.vector_type)   # e.g. "SINE"
print(info.complexity)    # e.g. "O(m^2)"
\end{lstlisting}
If no pattern matches, a \texttt{ValueError} is raised
rather than silently falling back, prompting the user
to switch to general state preparation explicitly.

\textbf{Example 2: Declared type with verification.}
The user believes the assembled vector is sinusoidal
and requests a guarantee before synthesis. PyEncode
fits frequency and phase from $\mathbf{f}$, verifies
the residual, and raises an informative error if the
check fails:
\begin{lstlisting}[style=python]
...
\end{lstlisting}
This is the \emph{correct by construction} path: a
circuit is produced only when $\mathbf{f}$ provably
matches the declared pattern.

\textbf{Example 3: Multi-tone signal.}
A signal processing application: a superposition of
two pure tones sampled on $N$ points is passed as a
vector. PyEncode's auto-detector tries the
\texttt{MULTI\_SINE} extractor, fits the two
frequencies and amplitudes, verifies the residual,
and returns an $\bigO{m^2}$ circuit:
\begin{lstlisting}[style=python]
from pyencode import encode_vector
import numpy as np
...
circuit, info = encode_vector(f)
# info.vector_type  -> "MULTI_SINE"
# info.complexity   -> "O(m^2)"
\end{lstlisting}

\loadfigs{multi_tone_vector.png}{multi_tone_circuit.png}
  {Multi-tone: $0.8\sin(3\pi i/N)+0.6\sin(7\pi i/N)$}
  {unused}
  {0.35}{sec5_multi_tone}

The same circuit would require $\bigO{2^m}$
gates under Shende's general preparation for $N=64$;
PyEncode produces it in $\bigO{m^2}$ gates.
...
\texttt{SINE(n=3, A=1.0)} or
\texttt{DISCRETE(k=32, P=1.0)} --- together with the
...
\loadfigs{params_discrete_vector.png}{params_discrete_circuit.png}
  {Discrete: $f_{32}=1$, $N=64$}
  {unused}
  {0.7}{sec5_params_discrete}

\begin{lstlisting}[style=python]
# Sinusoidal with phase
circuit, info = encode_params(
    SINE(n=3, A=2.0, phi=np.pi/4), N=64)
# info.complexity -> "O(m^2)"
\end{lstlisting}

\loadfigs{params_sine_vector.png}{params_sine_circuit.png}
  {Sine: $2\sin(3\pi i/N + \pi/4)$, $N=64$}
  {unused}
  {0.5}{sec5_params_sine}

\textbf{Example 2: Composite vector.}
A superposition of two sine modes and a discrete vector, expressed as a list of
constructor objects. Each component is synthesized
independently:
\begin{lstlisting}[style=python]
circuit, info = encode_params([
    SINE(n=3, A=1.0),
...
\end{equation}
arises as the canonical test problem in elliptic PDE
solvers and as an exact eigenfunction of the Laplacian
under Dirichlet boundary conditions~\cite{strang2007}.
After discretisation on an $N\times N$ grid, the
right-hand side vector has entries
$f_{ij} = \sin(n\pi i/N)\sin(m\pi j/N)$ and length $N^2$.

The key structural observation is that $\mathbf{f}$ is a
tensor product:
\begin{equation}
  \mathbf{f} = \mathbf{u} \otimes \mathbf{v}, \qquad
  u_i = \sin(n\pi i/N), \quad
  v_j = \sin(m\pi j/N).
\end{equation}
...
\loadfigs{poisson_vector.png}{poisson_circuit.png}
  {2D Poisson source: $\sin(2\pi i/N)\sin(3\pi j/N)$}
  {unused}
  {0.45}{sec6_poisson}
...
\loadfigs{finance_vector.png}{finance_circuit.png}
  {Price distribution as MULTI\_DISCRETE ($|S|\!=\!7$, $N\!=\!64$)}
  {unused}
  {0.45}{sec6_finance}
